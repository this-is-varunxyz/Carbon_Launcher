                ...
                children: [
                  StreamBuilder(
                    ...
                    builder: (context, snapshot) {
                      ...
                      final String minStr = now.minute.toString().padLeft(
                        2,
                        '0',
                      );
                  ...
                  builder: (context, state) {
                    ...
                    if (state is TaskLoaded) {
                      ...
                      final deadlines = tasks
                          .where(
                            (t) =>
                                t.type == TaskType.deadline && !t.isCompleted,
                          )
                          .toList();
                      final normalTasks = tasks
                          .where(
                            (t) => t.type == TaskType.normal && !t.isCompleted,
                          )
                          .toList();
                        ...
                        child: Column(
                          ...
                          children: [
                            if (deadlines.isNotEmpty) ...[
                              ...
                              ...deadlines.map(
                                (task) => _buildDeadlineTask(context, task),
                              ),
                              const SizedBox(height: 20),
                            ],

                            if (normalTasks.isNotEmpty ||
                                (deadlines.isEmpty && doneTasks.isEmpty)) ...[
                              _buildSectionHeader(
                                Icons.check,
                                'TASKS · ${normalTasks.length}',
                              ),
                              ...normalTasks.map(
                                (task) => _buildNormalTask(context, task),
                              ),
                              const SizedBox(height: 20),
                            ],
                            ...
                            if (doneTasks.isNotEmpty) ...[
                              ...
                              ...doneTasks.map(
                                (task) => _buildDoneTask(context, task),
                              ),
                            ],
      ...
      child: Row(
        children: [
          Icon(
            icon,
            size: 12,
            color: Colors.black.withOpacity(faded ? 0.15 : 0.32),
          ),
        ...
        child: StreamBuilder(
          ...
          builder: (context, snapshot) {
            ...
            if (task.deadline != null) {
              final totalMinutes = task.deadline!
              ...
              if (now.isAfter(task.deadline!)) {
                ...
                String remaining = diff.inDays > 0
                    ? '${diff.inDays}d'
                    : diff.inHours > 0
                    ? '${diff.inHours}h'
                    : '${diff.inMinutes}m';
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            if (task.priority != TaskPriority.none)
              _buildPriorityDot(_getPriorityColor(task.priority)),
          ],
        ),
      ),
    );
  ...
  Widget _buildCheckbox(BuildContext context, TaskEntity task) {
    return Container(
      ...
      child: task.isCompleted
          ? Icon(Icons.check, size: 10, color: Colors.black.withOpacity(0.4))
          : null,
    );
  }
